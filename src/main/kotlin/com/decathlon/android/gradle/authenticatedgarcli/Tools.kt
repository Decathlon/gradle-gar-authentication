...
internal fun ExecSpec.commandLineMultiplatform(vararg args: Any) {
    if (getCurrentOperatingSystem().isWindows) commandLine(
        *listOf("cmd", "/c").plus(args).toTypedArray()
    )
    else commandLine(*args)
}

internal inline operator fun <reified T : Any> Provider<T>.getValue(
    thisRef: Any?,
    property: KProperty<*>
): T = get()

@JvmName("getValueOptional")
internal inline operator fun <reified T : Any> Provider<Optional<T>>.getValue(
    thisRef: Any?,
    property: KProperty<*>
): T? = get().orElse(null)
